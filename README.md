...
フロントエンドよりも**バックエンドの設計・実装とテスト自動化**に重点を置いており、
単体テスト・結合テスト・E2Eテストのテストピラミッドを意識した構成と、GitHub Actions による CI の導入まで行っています。
...
> ⚠️ 無料プランのため、初回アクセス時はサーバーの起動に1～2分かかる場合があります。

---

## このアプリを作った理由

前職での Java 経験を活かしながら現代的なフレームワークを学び、開発工程を一通り経験するために制作しました。
ブランク期間を経た現在の技術力を客観的に示す目的もあり、 QA/SETエンジニアを目指す中で、
テスト設計書の作成・同値分割や境界値分析などの手法を用いたテストケースの洗い出し・単体テスト・結合テスト・E2Eテスト・CI導入まで継続的に取り組んでいます。
...
| E2Eテスト（予約フロー）           | 18件 | Playwrightを用いたE2Eテスト（Chromium・Firefox・WebKit）                         |
...
E2EテストではPlaywrightを採用し、ユーザー登録・認証フローと予約フローを3ブラウザで自動検証しています。  
Page Objectパターンで画面単位に操作を分離し、beforeEachでPageObjectの初期化とログインを共通化することで保守性を高めています。   
また、テストの独立性を確保するため、正常系のbeforeEachでテストごとに動的にユーザーを生成することで、テストデータの競合を防ぎ、3ブラウザでの並列実行を実現しています。   
GitHub Actions による CI を導入し、main ブランチへの push・PR をトリガーに単体テスト・結合テスト・E2Eテストが自動実行される環境を整えています。
